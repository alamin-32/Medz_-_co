...
const ManageItems = () => {
    ...
    return (
        <div>
            <h1 className='text-center text-3xl font-bold mt-5'>Add Product</h1>
            <form className='flex flex-col px-5  lg:w-3/6 mx-auto mt-6' onSubmit={handleSubmit(onSubmit)}>
                <input className=' font-bold text-white mb-1 p-3 rounded  bg-slate-500' name='img' placeholder='Photo Url' type="text" {...register("img")} />
                <input className=' font-bold mb-1 p-3 rounded bg-slate-500' name='name' placeholder='Product Name' {...register("name")} />
                <textarea className='font-bold mb-1 p-3 rounded bg-slate-500'
                    name='description' placeholder='Description' {...register("description")} />
                <input className='font-bold mb-1 p-3 rounded bg-slate-500' name='price' placeholder='Price' type="number" {...register("price")} />
                <input className='font-bold mb-1 p-3 rounded bg-slate-500' name='order' placeholder='Min Order' type="number" {...register("order")} />
